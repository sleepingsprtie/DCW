...
namespace DCW_Launcher
{
    public partial class Form_main : Form
    {
        ...
        private void btn_check_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(lastVersion) || string.IsNullOrEmpty(nowVersion))
            {
                MessageBox.Show("Config.xml is defect", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                ...
                if (last > now)
                {
                    if (MessageBox.Show("New version is available! Do you want to update?", "New version", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        string filePath = this.Download(this.server);
                        //更新
            ...
            using (WebClient client = new WebClient())
            {
                ...
                try
                {
                    ...
                    if (doc != null)
                    {
                        ...
                        if (node != null)
                        {
                            version = node.Attributes["version"].InnerText.Trim();
        ...
        private string Download(string server)
        {
            ...
            string fileURL = Application.StartupPath + "\\temp\\" + this.lastVersion + ".zip";
            FileStream fs = new FileStream(fileURL, FileMode.Create, FileAccess.Write, FileShare.ReadWrite);

            HttpWebRequest objWebRequest = (HttpWebRequest)WebRequest.Create(zipURL);
        ...
        private void btn_create_Click(object sender, EventArgs e)
        {
            ...
            xmlNode1.Attributes["path"].InnerText = this.tb_path.Text.Trim();
        ...
        {
            System.Diagnostics.Process.Start("DCW.exe", "");
            this.Close();
        }
